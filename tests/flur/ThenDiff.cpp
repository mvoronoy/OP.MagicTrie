...
namespace {
    ...
    void test_ThenAutoDifference(OP::utest::TestRuntime& tresult)
    {
        std::vector<std::string> unordered_minuend{ "aa", "aa", "bb", "bb", "bb", "c", "xx", "xx", "i", "jjjj", "klm" };
        std::shuffle(unordered_minuend.begin(), unordered_minuend.end(),
            tools::RandomGenerator::instance().generator());

        std::vector<std::string> unordered_subtrahend = { "aaa", "a", "bb", "bb", "d", "x", "z" };
        std::shuffle(unordered_subtrahend.begin(), unordered_subtrahend.end(),
            tools::RandomGenerator::instance().generator());

        std::multiset<std::string> ordered_minuend{ unordered_minuend.begin(), unordered_minuend.end() };
        std::multiset<std::string> ordered_subtrahend{ unordered_subtrahend.begin(), unordered_subtrahend.end() };

        std::multiset<std::string> expected;
        std::set_difference(
            ordered_minuend.begin(), ordered_minuend.end(), 
            ordered_subtrahend.begin(), ordered_subtrahend.end(),
            std::inserter(expected, expected.end()));

        tresult.assert_that<eq_sets>(
            expected,
            (src::of_container(ordered_minuend) >> then::auto_diff(src::of_container(ordered_subtrahend))),
            "o-o failed"
        );

        tresult.assert_that<eq_unordered_sets>(
            expected,
            src::of_container(ordered_minuend) >> then::auto_diff(src::of_container(unordered_subtrahend)),
            "o-u failed"
        );

        tresult.assert_that<eq_unordered_sets>(
            expected,
            src::of_container(unordered_minuend) >> then::auto_diff(src::of_container(unordered_subtrahend)),
            "u-u failed"
        );

        tresult.assert_that<eq_unordered_sets>(
            expected,
            src::of_container(unordered_minuend) >> then::auto_diff(src::of_container(ordered_subtrahend)),
            "u-o failed"
        );
    }

    static auto& module_suite = OP::utest::default_test_suite("flur.then.diff")
        ...
        .declare("adaptive", test_ThenAutoDifference)
        ;
