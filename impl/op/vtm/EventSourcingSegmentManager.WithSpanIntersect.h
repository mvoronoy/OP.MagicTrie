    ...
    namespace trie
    {
        ...
        public:
            ...
            */
                ...
                FarAddress pos, segment_pos_t size, WritableBlockHint hint = WritableBlockHint::update_c)  override
            {
                ...
                std::shared_lock guard_history_ro(_global_history_acc); //shared ro access are enough to iterate
                auto [h_beg, h_end] = narrow_history_log(search_range, new_block_iter);
        ...
        private:
            ...
            */
            ...
            using region_map_t = // std::multimap<far_pos_t, history_iterator_t>;
                                OP::zones::SpanMap<RWR, history_iterator_t>;

            history_t _global_history;
            region_map_t _history_region_index;
            ...
            */ 
                ...
                RWR search_range, ReadonlyMemoryChunk&& raw)
            {
                ...
                auto [all_iter, h_end] = narrow_history_log(search_range, _global_history.end());
                // find all previously used block that have any intersection with query
            ...
            {
                auto current_isolation = _read_isolation.load();
                auto [all_iter, h_end] = narrow_history_log(search_range, std::move(end));
                // find all previously used block that have any intersection with query
                // to check if readonly block is allowed
                for (; all_iter != h_end; ++all_iter)
                {
                ...
                while(!_stop_garbage_collector.load())
                {
                    ...
                    if(!_ready_to_dispose.empty())
                    {
                        ...
                        for (auto& block_iter : tran->_transaction_log)
                        {
                            ...
                            auto pos_from = _history_region_index.intersect_with(block._range);
                            for(auto pos_to = _history_region_index.end(); pos_from != pos_to; ++pos_from)
                            {
                                if( pos_from->second == block_iter )
                                {
                                    _history_region_index.erase(pos_from);
                                    break; //only 1 entry possible
                                }
                            }
                ...
                ShadowBuffer buffer, std::uint8_t flag)
            {
                _global_history.emplace_back(
                    ...
                    type,
                    transaction_id,
                    ++ _epoch,  //note epoch starts with 1
                    std::move(buffer),
                    flag
                );
                ...
                --new_block_iter;
                _history_region_index.emplace(search_range, new_block_iter);
                return new_block_iter;
            }

            std::pair<history_iterator_t, history_iterator_t> narrow_history_log(const RWR& search_range, history_iterator_t end) 
            {
                auto blocks_in_touch = _history_region_index.intersect_with(search_range);
                history_iterator_t begin = end;
                if(blocks_in_touch == _history_region_index.end())
                { //no intersections
                    //return std::pair{ end, end };
                }
                else
                {
                    //find with lowest epoch value
                    std::uint64_t min_epoch = std::numeric_limits<std::uint64_t>::max(); //real epoch starts with 1
                    for (auto touch_end = _history_region_index.end(); blocks_in_touch != touch_end; ++blocks_in_touch)
                    {
                        if(blocks_in_touch->second->_epoch < min_epoch)
                        {
                            min_epoch = blocks_in_touch->second->_epoch;
                            begin = blocks_in_touch->second;
                        }
                    }
                }
