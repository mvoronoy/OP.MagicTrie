    ...
    namespace trie {
        ...
        */
        ...
        public:
            ...
            transaction_ptr_t begin_transaction() override
            {
                ...
                if (insres.second) //just insert
                {
                    ...
                    return insres.first->second.static_pointer_cast<Transaction>();
                }
            ...
            MemoryChunk writable_block(FarAddress pos, segment_pos_t size, WritableBlockHint hint = WritableBlockHint::update_c)  override
            {
                ...
                return MemoryChunk(include_delta, found_res->second.shadow_buffer().ghost(), size, pos);
            }
        ...
        private:
            ...
            /** Write explicitly to segment manager */
            ...
            struct BlockUse
            {
                    ...
                    WritableBlockHint hint = WritableBlockHint::block_no_hint_c)
                {
                    ...
                    _shadow_buffer = shadow_buffer_t{ new std::uint8_t[size], size, true };
                ...
                void update_shadow(const MemBlock& new_mem, segment_pos_t old_size, segment_pos_t relative_offset = 0)
                {
                    ...
                    shadow_buffer_t new_buffer = shadow_buffer_t{
                        new std::uint8_t[new_mem.count()], new_mem.count(), true };
            ...
            struct SavePoint : public Transaction
            {
                ...
                using log_t = std::deque<typename captured_blocks_t::iterator>;
                using iterator = typename log_t::iterator;
                ...
                {
                    _framed_tran->_owner->apply_transaction_log(transaction_id(), begin(), end(), false);
            ...
            struct TransactionImpl : public Transaction
            {
                ...
                */
                ...
                {
                    _save_points.emplace_back(new SavePoint(this));
                    return _save_points.back().static_pointer_cast<Transaction>();
                }
                ...
                void finish_transaction(bool is_commit)
                {
                    ...
                    {
                        auto& slog = *ri;
                        _owner->apply_transaction_log(transaction_id(), slog->begin(), slog->end(), is_commit);
                    }
                    _save_points.clear();
                    //wait until transaction is erased
                    erase_promise.wait();
                }
                TransactedSegmentManager* _owner;

                typedef std::unordered_map<const std::uint8_t*, far_pos_t> address_lookup_t;
                typedef std::deque<vtm::TransactionPtr<SavePoint>> save_point_stack_t;
            ...
            using transaction_impl_ptr_t = vtm::TransactionPtr<TransactionImpl>;
            ...
            template <class TSaveIter>
            void apply_transaction_log(Transaction::transaction_id_t tid, TSaveIter iter, TSaveIter iter_end, bool is_commit)
            {
            ...
            */
            ...
            void extend_ro_memory_block(Iterator& to_extend, RWR& new_range, ReadonlyBlockHint hint, transaction_impl_ptr_t& current_tran)
            {
                ...
                {
                    new_buffer = shadow_buffer_t{
                        new std::uint8_t[new_range.count()], new_range.count(), true };
                ...
                new_block.set_shadow(std::move(new_buffer));
            ...
            */
                ...
                captured_blocks_t::iterator& found_res)
            {
                ...
                //because if write=access acquire the same transaction must always see changes
                ...
                auto include_delta = pos - found_res->first.pos();//handle case when included region queried
                return ReadonlyMemoryChunk(
                    include_delta, found_res->second.shadow_buffer().ghost(), //use shadow
                    size, std::move(pos));
            }
